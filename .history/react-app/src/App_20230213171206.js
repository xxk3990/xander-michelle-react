...
export default function App() {
  ...
  if(centurySliderValue > 800) {
    if(meteorites.year === undefined) {
      return;
    }
    centurySortedmeteorites = meteorites.filter(meteorite => meteorite.year.substring(0,4) >= centurySliderValue && meteorite.year.substring(0,4) <= centurySliderValue + 99)
 ...
 // let totalCountValue = 0;
  ...
  const currentmeteoriteData = useMemo(() => {
    ...
    //let slicedFilter;
    // if(searchInput !== "") {
    //   slicedFilter = searchedmeteorites.slice(firstPageIndex, lastPageIndex);
    // } else if(centurySliderValue > 800) {
    //   slicedFilter = centurySortedmeteorites.slice(firstPageIndex, lastPageIndex);
    // }
    //ISSUE 12/25/22 – CANNOT DO MORE TYPES OF FILTERING IF CURRENTmeteoriteDATA IS SLICING USING SEARCHEDmeteoriteS.
    return searchedmeteorites.slice(firstPageIndex, lastPageIndex), centurySortedmeteorites.slice(firstPageIndex, lastPageIndex)
      ...
      console.log("searchInput from App.js", searchInput)
        ...
        return (
          <div className="App">
              ...
              totalCount={searchedmeteorites.length} //WON'T BE ABLE TO HAVE PAGINATION CHANGE FOR MULTIPLE FILTERS IN CURRENT SETUP
              pageSize={PageSize}
