...
export default function App() {
  ...
  const [centurySliderValue, setCenturySliderValue] = useState(799); 
  ...
  if(centurySliderValue > 800) {
    centurySortedmeteorites = meteorites.filter(meteorite => {
      if(meteorite.year === undefined) {
        ...
        */
        //let parseYear = 
        const yearFirst = centurySliderValue.toString().substring(0);

        console.log('yearArray', yearFirst);
        if(yearFirst === '0') {
 ...
 // let totalCountValue = 0;
  ...
  if(meteorites === undefined) {
    ...
    //no meteoriteites fell during these timeframes, so display notice to user
      return (
      <div className="App">
        <Pagination
        ...
        <output>{centurySliderValue} - {centurySliderValue / 100}99</output> {/* should be good until the year 10000 AD :D */}
        <h4>No meteorites fell during the selected timeframe.</h4>
      </div>
      )
  } else { 
      console.log("searchInput from App.js", searchInput)
        ...
        return (
          <div className="App">
            ...
            />
            <input type="range" min="799" max="2200" step="100" className="century-slider" onChange={centurySliderHandler}/>
            <output>{centurySliderValue === 799 ? 'Drag to filter by centuries' : `${centurySliderValue} - ${centurySliderValue / 100}99` } </output> {/* should be good until the year 10000 AD :) */}
            <section className = "data-grid">
              {currentmeteoriteData.map(m => {
               
                  return <meteoriteCard m={m} id={m.id} />
