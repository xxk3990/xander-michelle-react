  ...
  if(centurySliderValue > 800) {
    centurySortedMeteorites = meteorites.filter(meteorite => {
      if(meteorite.year === undefined) {
        ...
        */
        //let parseYear = 
        const yearFirst = meteorite.year.substring(0,1);
       // console.log('yearFirst before if', yearFirst);
        if(yearFirst[0] === '0') {
          ...
          if(parseInt(yearFirst[0]) >= parseSlider && parseInt(yearFirst[0]) < parseSlider + 1) { //check if year is between slider value and next highest one
            return meteorites;
          }
        } else {
