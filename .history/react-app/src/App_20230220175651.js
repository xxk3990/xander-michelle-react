  ...
  if(centurySliderValue > 800) {
    centurySortedmeteorites = meteorites.filter(meteorite => {
      if(meteorite.year === undefined) {
        ...
        0800, 0900
        */
        let parseYear = parseInt(meteorite.year.substring(0,4));
        const yearFirstNumber = String(parseYear)[0];

        console.log('yearArray', yearFirstNumber);
        if(yearFirstNumber[0] === '0') {
          console.log('first', yearFirstNumber);
          const parseSub = parseInt(meteorite.year.substring(0,1));
