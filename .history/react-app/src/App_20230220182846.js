  ...
  if(centurySliderValue > 800) {
    centurySortedmeteorites = meteorites.filter(meteorite => {
      if(meteorite.year === undefined) {
        ...
        */
        //let parseYear = 
        const yearFirstNumber = parseInt(meteorite.year.substring(0,1));

        console.log('yearArray', yearFirstNumber);
        if(yearFirstNumber !== 1 && yearFirstNumber !== 2) {
          console.log('first', yearFirstNumber);
          const parseSlider = parseInt(centurySliderValue.toString().substring(0,1));
          if(yearFirstNumber >= parseSlider && yearFirstNumber < parseSlider + 1) { //check if year is between slider value and next highest one
            return meteorites;
          }
        } else {
          ...
          <div className="App">
            ...
            <section className = "data-grid">
              {currentmeteoriteData.map(m => {
                switch (centurySliderValue) {
                  case 800:return <meteoriteCard m={m} key={m.id}/>
                  case 900: return <meteoriteCard m={m} key={m.id}/>
                  case 1000: return <h4> No meteoriteites during the {centurySliderValue}s</h4>
                  case 1100: return <h4> No meteoriteites during the {centurySliderValue}s</h4>
                  case 1200: return <h4> No meteoriteites during the {centurySliderValue}s</h4>
                  case 1300: return <h4> No meteoriteites during the {centurySliderValue}s</h4>
                  case 1400: return <meteoriteCard m={m} key={m.id}/>
                  case 1500: return <meteoriteCard m={m} key={m.id}/>
                  case 1600: return <meteoriteCard m={m} key={m.id}/>
                  case 1700: return <meteoriteCard m={m} key={m.id}/>
                  case 1800: return <meteoriteCard m={m} key={m.id}/>
                  case 1900: return <meteoriteCard m={m} key={m.id}/>
                  case 2000: return <meteoriteCard m={m} key={m.id}/>
                  default: return
                }
              })
            }
